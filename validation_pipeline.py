...
##
def poisson(predictions, test_lables):
    metric = tf.keras.metrics.Poisson()
    metric.update_state(predictions, test_labels)
    return metric.result().numpy()

def categoricalAccuracy(predictions, test_lables):
...
def MetricsClass0(predictions, test_labels):
  ...
  while i < predictions.size:
    if predictions[i] == 00 and test_labels[i] == 00:
  ...
  if TruePositives >= 1:
    ...
    return(TruePositives, FalsePositives, FalseNegatives, round(Precision,3), round(Recall,3), round(F1_score, 3))
  else:
...
def microF1(metrics42, metrics41, metrics40, metrics39, metrics38, metrics37, metrics36, metrics35, metrics34, metrics33, metrics32, metrics31, metrics30, metrics29, metrics28, metrics27, metrics26, metrics25, metrics24, metrics23, metrics22, metrics21, metrics20, metrics19, metrics18, metrics17, metrics16, metrics15, metrics14, metrics13, metrics12, metrics11, metrics10, metrics9, metrics8, metrics7, metrics6, metrics5, metrics4, metrics3, metrics2, metrics1, metrics0):
  ...
  microF1 = (2 * ((globalPrecision * globalRecall)/(globalPrecision + globalRecall)))
  return(round(microF1, 2))
...
#gewichteter F1 Score nach Anzahl der Beispiele die drin sind, macht in diesem Fall kein Sinn, da alle Beispiele gleich oft drin sind
...
def weightedF1score2(metrics42,metrics41,metrics40,metrics39, metrics38, metrics37, metrics36, metrics35, metrics34, metrics33, metrics32, metrics31, metrics30, metrics29, metrics28, metrics27, metrics26, metrics25, metrics24, metrics23, metrics22, metrics21, metrics20, metrics19, metrics18, metrics17, metrics16, metrics15, metrics14, metrics13, metrics12, metrics11, metrics10, metrics9, metrics8, metrics7, metrics6, metrics5, metrics4, metrics3, metrics2, metrics1, metrics0):                
  weightedF1 = (metrics0[5] + metrics1[5] + metrics2[5] + metrics3[5] + metrics5[5] + metrics4[5] + metrics6[5] + metrics7[5] + metrics8[5] + metrics9[5] + metrics10[5] + metrics11[5] *2 + metrics12[5] *2 + metrics13[5] *2 + metrics14[5] *2 + metrics15[5] *2 + metrics16[5] + metrics17[5] *2 + metrics18[5] + metrics19[5] + metrics20[5] + metrics21[5] + metrics22[5] + metrics23[5] + metrics24[5] + metrics25[5] + metrics26[5] + metrics27[5] + metrics28[5] + metrics29[5] + metrics30[5] + metrics31[5] + metrics32[5] + metrics33[5] + metrics34[5] + metrics35[5] + metrics36[5] + metrics37[5] + metrics38[5] + metrics39[5] + metrics40[5] + metrics41[5] + metrics42[5])/(49)
  return(round(weightedF1,3))

#Klasse 0 = Tempo 20
#Klasse 1 = Tempo 30
#Klasse 2 = Tempo 50, Klasse 3 = Tempo 60, Klasse 4 = Tempo 70, Klasse 5 = Tempo 80, Klasse 6 = 80 aufgehoben, kLasse 7 = tempo 100, Klasse 8 = Tempo 120, Klasse 9 = nicht überholen
